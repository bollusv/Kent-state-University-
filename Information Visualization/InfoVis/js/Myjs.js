...
map.on('draw:created', function (e) {
	...
	if (type === 'rectangle') {
        ...
        // console.log([[bounds.getSouthWest().lng,bounds.getSouthWest().lat],[bounds.getNorthEast().lng,bounds.getNorthEast().lat]])
...
function DrawRS(trips) {
    ...
    // rt.data([[-180,-90],[180,90]]).
    //    then(function(d){
    //         // console.log('in here');
    //         var all = d.map(function(a) {return a.properties;});
    //         places_count(all);
    //     });
    clearbar();
    barplot(trips);
    ...
    scatterplot(trips);
    clearpie();
    draw_source_pie(trips);
    draw_destination_pie(trips);
    clearbar3();
    bar3plot(trips);
}